     ...
     forkJoin([response1
    ]).pipe(map(([tickets]) => tickets.map(ticket => 
        {
          return {
            "id": ticket.id,
            "price": ticket.price,
            "fromIdPlace": ticket.fromIdPlace,
            "toIdPlace": ticket.toIdPlace,
            "from": this.places.find(x=> x.id == ticket.fromIdPlace)?.name,
            "to": this.places.find(x=> x.id == ticket.toIdPlace)?.name,
            "name": "",
            "dateFrom": new Date(),
            "phoneNumber": ""
          } 
